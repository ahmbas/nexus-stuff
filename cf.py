...
# MGMT Provdider ID to skip
EXCLUDED_PROVIDER_IDS = ["10000000000001"]
try:
    import json
except ImportError:
...
class CloudFormsInventory(object):
    ...
    def read_settings(self):
        ...
        self.cloudforms_username =  os.environ.get('CFME_USER')
        self.cloudforms_pw =  os.environ.get('CFME_PASSWORD')
        self.cloudforms_url =  os.environ.get('CFME_ENDPOINT')
    ...
    def _get_hosts(self):
        ...
        while not last_page:
            offset = page * limit
            ret = self._get_json("%s/api/vms?offset=%s&limit=%s&expand=resources,tags,hosts,custom_attributes,&attributes=ipaddresses,ext_management_system" % (self.cloudforms_url, offset, limit))
    ...
    def update_cache(self):
        ...
        for host in self._get_hosts():
            if not host.get("ext_management_system", {}):
                continue
            if host.get("ext_management_system") and host["ext_management_system"].get("id", None) in EXCLUDED_PROVIDER_IDS:
                continue
            ...
            if host.get("custom_attributes"):
                mgmt_ip = filter(lambda x: x['name'] == 'floating_ip' ,host['custom_attributes'])
                if mgmt_ip:
                    host["ansible_ssh_host"] = mgmt_ip[0]["value"]
